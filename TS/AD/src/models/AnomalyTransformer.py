...
class AnomalyTransformer(nn.Module):
    def __init__(self, cfg):
        ...
        self.anomaly_ratio = cfg.anomaly_ratio
    ...
    def forward(self, input, input_timestamp, target, criterion, cal_score=False):
        """
        목적: Anomaly Transformer의 forward pass 및 minimax loss 계산
        조건
        - input: [B, L, F] 입력 시계열
        - target: [B, L, F] 타겟 (이상탐지에서는 input과 동일)
        - criterion: reconstruction loss (MSELoss)
        - cal_score: True일 경우 anomaly score 반환
        - 학습 시 minimax strategy를 위해 (loss1, loss2) 튜플 반환
        """
        ...
        # Minimax strategy: loss1 minimizes, loss2 maximizes association discrepancy
        loss1 = rec_loss - self.k * series_loss
        loss2 = rec_loss + self.k * prior_loss

        if cal_score:
            score = self.cal_anomaly_score(output, input, series, prior)
            return output, (loss1, loss2), score

        return output, (loss1, loss2)

    def cal_anomaly_score(self, output, input, series_list, prior_list):
        """
        목적: Association Discrepancy 기반 anomaly score 계산
        조건
        - reconstruction loss와 attention discrepancy를 결합하여 이상 점수 산출
        - temperature scaling 적용
        """
        criterion = nn.MSELoss(reduction='none')
        rec_loss = torch.mean(criterion(output, input), dim=-1)  # [B, L]

        series_loss = 0.0
        prior_loss = 0.0
        for u in range(len(prior_list)):
            normalized_prior = prior_list[u] / torch.unsqueeze(
                torch.sum(prior_list[u], dim=-1), dim=-1
            ).repeat(1, 1, 1, self.win_size)

            if u == 0:
                series_loss = my_kl_loss(series_list[u], normalized_prior.detach()) * self.temperature
                prior_loss = my_kl_loss(normalized_prior, series_list[u].detach()) * self.temperature
            else:
                series_loss += my_kl_loss(series_list[u], normalized_prior.detach()) * self.temperature
                prior_loss += my_kl_loss(normalized_prior, series_list[u].detach()) * self.temperature

        metric = torch.softmax((-series_loss - prior_loss), dim=-1)  # [B, L]
        score = (metric * rec_loss).detach().cpu().numpy()  # [B, L]
        return score
