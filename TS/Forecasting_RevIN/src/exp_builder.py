...
def training_dl(
    ...
    state = {"best_epoch": -1, "best_step": -1, f"best_{ckp_metric}": np.inf}
    eval_results = {}
    ...
    for epoch in range(epochs):
        ...
        for idx, item in enumerate(trainloader):
            ...
            input_ts, target_ts, input_mark, target_mark = item

            input_ts = input_ts.float().to(accelerator.device)
            target_ts = target_ts.float().to(accelerator.device)
            input_mark = input_mark.float().to(accelerator.device)
            target_mark = target_mark.float().to(accelerator.device)

            # forward
            outputs = model(x_enc=input_ts,
            ...
            if use_wandb and ((wandb_iteration + 1) % wandb_iter == 0):
                train_results = OrderedDict([
                    ...
                    ('train_loss_step',losses_m.avg)
                ])
                wandb.log(train_results, step=wandb_iteration)

        if use_wandb:
            train_results_epoch = OrderedDict([
                ('lr', optimizer.param_groups[0]['lr']),
                ('train_loss_epoch', losses_m.avg),
                ('epoch', epoch + 1),
            ])
            wandb.log(train_results_epoch, step=wandb_iteration)
...
def test_dl(model, dataloader, criterion, accelerator: Accelerator, 
            ...
            model_name: str, name: str = 'TEST', return_output: bool = False, dataset=None) -> dict:
    ...
    with torch.no_grad():
        for idx, item in enumerate(dataloader):
            ...
            # inverse transform (TEST에서만)
            if name.upper() == "TEST" and dataset is not None:
                shape = outputs.shape
                outputs = dataset.inverse_transform(outputs.squeeze(0)).reshape(shape)
                target_ts = dataset.inverse_transform(target_ts.squeeze(0)).reshape(shape)
    ...
    metrics = cal_metric(total_outputs, total_targets)
